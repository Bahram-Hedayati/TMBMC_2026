...
results_hybrid_entropy = []        # Stores the effectiveness value of the NMA for each ensemble simulation
results_tumors_hybrid_entropy = [] # Stores the number of cancerous voxels remained at the end of each ensemble simulation
results_hybrid_states_entropy = [] # For each ensemble simulation, it stores the list of the NMA's state over each ensemble simulation
...
for e in np.arange(1, E + 1): # This loop handles ensemble simulations
    ...
    if e % 10 == 0:
        mean_effectiveness = (np.mean(results_hybrid_entropy) / np.sum(tumor_mask)) * 100
    ...
    for t in np.arange(1, time_steps + 1): # This loop handles oxygen diffusion and the NMA's movement
        ...
        if sc.state == "Chemotaxis": # The chemotaxis-based state
            ...
            # The next direction of the NMA is chosen by obtaining the directions, whose the entropy of the voxels in its
            # patch is maximum compared to the entropy values of other directions.
    ...
    results_hybrid_entropy.append(sc.released_drug)                 # Stores the number of treated voxels over each ensemble simulation
    results_tumors_hybrid_entropy.append(env_hybrid.cancer_voxels)  # Stores the number of remained cancerous voxels over each ensemble simulation
    results_hybrid_states_entropy.append(results_states)            # Stores the switching dynamics between the chemotaxis-based and the entropy-based states

# Save results into csv files
import csv

# Save results_hybrid_entropy to a CSV file
with open('results_hybrid_mutiple_tumors_entropy.csv', 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(results_hybrid_entropy)

# Save results_tumors_hybrid_entropy to a CSV file
with open('results_tumors_hybrid_mutiple_tumors_entropy.csv', 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerows(results_tumors_hybrid_entropy)

# Save results_hybrid_states_entropy to a CSV file
with open('results_states_hybrid_mutiple_tumor_entropy.csv', 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerows(results_hybrid_states_entropy)
