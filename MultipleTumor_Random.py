...
grid_size, tumor_mask, substrate = tme_multi_tumor_single_agent_init()
...
T = 2000 * 60              # Total simulation time per run (in seconds)
...
results_random = []        # Stores the effectiveness value of the NMA for each ensemble simulation
results_tumors_random = [] # Stores the number of cancerous voxels remained at the end of each ensemble simulation

for e in np.arange(1, E + 1): # This loop handles each ensemble simulation
    ...
    env_random = Environment_Class(grid_size, grid_size, substrate.copy(), tumor_mask.copy()) # Define the environment (TME) object

    # Report the mean effectiveness every 10 ensmble simutations
    if e % 10 == 0:
        mean_effectiveness = (np.mean(results_random) / np.sum(tumor_mask)) * 100
    ...
    for t in np.arange(1, time_steps + 1): # This loop handles oxygen diffusion and the NMA's movement
        ...
        laplacian = laplace(env_random.bgf, mode='constant', cval=38.0) # Laplacian operator represents the diffusion process
        diffusion_term = D * laplacian / dx ** 2                        # Diffusion term for further usage
        decay_term = -lambda_decay * env_random.bgf * env_random.tumors # Decay oxygen pressure only in tumor voxels
        env_random.bgf += dt * (diffusion_term + decay_term)            # Explicit update
        env_random.bgf = np.clip(env_random.bgf, 0, 38.0)               # Clamp to physical range of oxygen pressure
        ...
        sc.direction = random.choice(directions) # Choose the next direction randomly

        # Movement
        # Applying boundary conditions for the NMA: it reflects if it reaches the boundary of the lattice
        sc.direction = env_random.check_reflection(sc.direction[0], sc.direction[1], sc.x, sc.y, grid_size)
        ...
        sc.bgf = env_random.bgf[x][y]                # Senses the substrate value inside the current voxel
        sc.find_tumor = env_random.tumors[x][y] == 1 # Checks whether the current voxel is cancerous or not
        ...
        if sc.find_tumor: # If the NMA recognizes a cancerous voxel
            ...
            env_random.tumors[x][y] = 0 # Removes the diagnosed cancerous voxel from the tumor mask
            env_random.bgf[x][y] = 38.0 # Rising the oxygen value inside the voxel to the maximum, due to releasing drug

        # Stores the number of remained cancerous voxels at the end of the current iteration
        env_random.cancer_voxels.append(np.sum(env_random.tumors))

    results_random.append(sc.released_drug)                # Stores the number of treated voxels over each ensemble simulation
    results_tumors_random.append(env_random.cancer_voxels) # Stores the number of remained cancerous voxels over each ensemble simulation

# Save results into csv files
import csv

# Save results_random to a CSV file
with open('results_random_multiple_tumor.csv', 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(results_random)

# Save results_tumors_random to a CSV file
with open('results_tumors_random_multiple_tumor.csv', 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerows(results_tumors_random)
